...
export const Key = ({ keyOne, keyTwo }: KeyProps) => {
  ...
  return (
    <div className="mt-auto px-4 py-3 rounded-[12px] shadow-custom-md transition-colors duration-200 ease-in-out bg-[#262928] group-hover:bg-[#313534] text-sm">
      ...
      <div className="inline-flex gap-2 items-center w-full">
        ...
        <Copy onClick={() => handleCopy(keyOne)} className="w-4 h-4 inline-block ml-auto cursor-pointer hover:scale-110 transition-all duration-75 ease-out" />
      </div>
      <div className="mt-1 inline-flex gap-2 items-center w-full">
        ...
        <Copy onClick={() => handleCopy(keyTwo)} className="w-4 h-4 inline-block ml-auto cursor-pointer hover:scale-110 transition-all duration-75 ease-out" />
      </div>
    </div>
  )
}